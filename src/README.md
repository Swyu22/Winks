...
| `components/` | 纯 UI 组件；`ModalDialog` 统一原生 dialog，`TaxonomyEditor` 统一分类/标签编辑，不直接调用 Supabase |
...
该命令使用 Node 内置 test runner，覆盖 `linkMeta`、`linkActions` 与 `linksCache` 的纯函数合同（共 21 个测试）。完整本地门禁使用 `npm run verify`。
