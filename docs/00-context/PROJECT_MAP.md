...
└─────────┼────────────────────┼────────────────────┼──────────────┘
          │                    │ HTTPS (anon key)   │
          ▼                    ▼                    ▼
  PinModal / LinkModal   Supabase Postgres    links.category meta
  LinkCard / Sidebar     table: public.links  + links.description
```
...
| [src/components/LinkCard.jsx](../../src/components/LinkCard.jsx) | 链接卡展示、可选单行简介、favicon 多源级联、28px 复制图标按钮、编辑/删除/打开回调 | 不直接修改 links |
| [src/components/LinkModal.jsx](../../src/components/LinkModal.jsx) | 新增/编辑表单、可选简介输入、标签/分类就地增删入口 | 不直接写数据库 |
...
| [src/lib/constants.js](../../src/lib/constants.js) | `DEFAULT_*`、`LINK_META_PREFIX`、`LINK_DESCRIPTION_MAX_LENGTH`、`APP_VERSION`、运行模式、PIN、筛选、缓存常量 |
| [src/lib/linkMeta.js](../../src/lib/linkMeta.js) | meta 编解码；`hydrateLink` 统一简介/clicks/legacy 行；tag/classification/board 归一化；favicon 与分类排序 |
| [src/lib/linkActions.js](../../src/lib/linkActions.js) | 纯函数：分类/标签本地变更、链接保存载荷归一化；空简介转换为数据库 `null`。可被 Node 测试 |
...
| [supabase/schema.sql](../../supabase/schema.sql) | `links` 表 schema（含 description/clicks 约束与索引）+ 完全开放的 RLS 策略；当前迁移结果快照 |
...
useLinks.useEffect → fetchLinks（不再 setLoading(true)，避免覆盖已渲染内容）
  ...
  → data.map(hydrateLink)        ← 解析 meta + 简介/点击数/legacy 兜底
...
LinkModal.onSave → useLinks.handleSaveLink
  → 规范化 description / tags / classification / board
  → 空 description 写 null；encodeLinkMeta(...) 写 links.category
...
- **分类合同只通过 `links.category` 流通**；简介使用独立 `links.description` 列，禁止混入 meta。改任一字段前先读 [data-model.md](../10-spec/data-model.md)。
...
| Description（简介） | 独立可空列；前端展示为 0 至 15 个字符的单行字符串 |
| Hydrate | 把数据库行解析成前端使用的 `{board, category, tags, description, clicks}` 形状 |
