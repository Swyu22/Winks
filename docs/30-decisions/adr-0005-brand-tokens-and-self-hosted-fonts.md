...
- **状态**：Accepted（功能对比度规则已被 [ADR-0006](adr-0006-wcag-aa-interaction-baseline.md) 部分取代）
- **日期**：2026-06-26
- **作者**：Claude（Opus 4.8）
...
**历史规则（2026-06-26）**：品牌实底 `#FFD000` 曾统一配白色图形/文字。2026-07-13 起按 [ADR-0006](adr-0006-wcag-aa-interaction-baseline.md) 修订：功能文字/图标改用 `--brand-foreground` 近黑；白色保留给非交互 Logo / favicon 品牌图形，以及 `aria-hidden` 的装饰性首字母 fallback。
...
> 演化记录中的白字/品牌黄对比度约 `1.47:1`，不满足 WCAG AA；该已知风险现已由 ADR-0006 闭环。
...
4. **静态资源不读 CSS 变量**：[public/favicon.svg](../../public/favicon.svg) 用字面 `#FFD000` + 白色品牌闪电；改品牌色时需手动同步。
