-- Baseline migration: snapshot of the live Winks schema (links table + open RLS).
-- Mirrors supabase/schema.sql. Fully idempotent (CREATE/ALTER ... IF [NOT] EXISTS), so a
-- `supabase db push` against the already-provisioned remote is a safe no-op that simply
-- records this migration in the history.
