...
3. **PIN `5185` 硬编码在前端**（[src/components/PinModal.jsx](src/components/PinModal.jsx)），后端 RLS 完全开放——这是「协作式弱保护」选型，详见 [adr-0002](docs/30-decisions/adr-0002-frontend-pin-only-auth.md)。
...
| Hook | 行为 | 失败处理 |
|---|---|---|
| `pre-commit` | 运行 `npm run lint` | 修复 lint 错误，**不要** `--no-verify` 绕过 |
